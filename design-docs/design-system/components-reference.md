...
- name: Toggle
  status: nova-aligned
  figma_synced: true                            # Erstport 2026-06-22 (Figma + Code zusammen gebaut)
  source: { registry: "@shadcn", item: toggle, style: radix-nova }   # co-portiert mit toggle-group (dessen Items toggleVariants importieren)
  code:
    dir: libs/ui/src/components/ui/toggle/
    exports: [Toggle, toggleVariants, ToggleProps, ToggleVariant, ToggleSize]
    barrel: "libs/ui/src/index.ts → export * from './components/ui/toggle'"
  figma:
    section: { name: "Toggle", id: "4374:2289" }
    set: { name: "Toggle", id: "4375:2379" }       # 30 Member, 3 Achsen, WRAP-Grid
    axis: { variant: [default, outline], size: [default, sm, lg], state: [default, hover, on, focus, disabled] }   # state = Figma-only (Code: on/off via aria-pressed/data-state, hover/focus/disabled = CSS-Pseudo; KEIN state-Prop → NICHT als CVA zurücksyncen)
    vars: { muted-fill: "3037:12", ink: "3037:3", input-border: "4197:9644", ring: "3038:6", corner-lg: "3073:4", corner-md: "3073:3" }
    focus_glow: "literal DROP_SHADOW radius:0 spread:3 rgba(74,85,98,.5)=ring/50 sbn:false clip:true — verbatim von .Input focus 3176:305"
    examples: { group: "Usage Examples 4379:2313 (Variants / Sizes / States)" }
  skill: /shadcn-component-port (+ /figma-build-rules, 2026-06-22)
  notes: >
    Single-Element CVA (Radix Toggle.Root, radix-ui Umbrella-Import behalten = volles Primitive, Dialog/Select-
    Konvention, B8/B13). Achsen variant [default,outline] × size [default,sm,lg]; on/off = Radix Pressed-State
    (aria-pressed FÜR Group-Items + data-[state=on] standalone — BEIDE als bg-muted-fill behalten), KEIN CVA-Axis.
    DS-Clothing: rounded-lg→corner-lg (corner-lg `use` nennt „Toggles"); sm rounded-[min(--radius-md,12px)]→corner-md;
    text-sm font-medium→text-format-label; hover bg-muted→bg-muted-fill, text-foreground→text-ink; on/hover-Fill
    bg-muted→bg-muted-fill; outline border-input→border-input-border; gap-1→gap-xs, pr-2/pl-2→pr-md/pl-md, pr-1.5/
    pl-1.5→pr-sm/pl-sm. text-[0.8rem] (sm) GEDROPPT (kein sub-14 Sans-Rung, B21/B23 → Rolle text-format-label). px-2.5
    (10px, kein Rung) + h-8/7/9 + min-w + size-4/3.5 numerisch. ring-[3px] (B15). dark: entfernt. aria-invalid bindet
    destructive ⚠-Platzhalter. Lokale Named-Union-Typen (ToggleVariant/ToggleSize) `satisfies Record<…>` → docgen
    löst die Unions auf. figma-verify CLEAN (0 text-as-icon, 30 Vektor-Icons). Stories Default(play, userEvent)/
    Variants/Sizes/States; jsdom-Spec (Radix-Button, kein Portal → kein Polyfill). Gate grün.

- name: ToggleGroup
  status: nova-aligned
  figma_synced: true                            # Erstport 2026-06-22 (Figma + Code zusammen gebaut)
  source: { registry: "@shadcn", item: toggle-group, style: radix-nova }
  code:
    dir: libs/ui/src/components/ui/toggle-group/
    exports: [ToggleGroup, ToggleGroupItem, ToggleGroupProps, ToggleGroupItemProps]
    barrel: "libs/ui/src/index.ts → export * from './components/ui/toggle-group'"
  figma:
    section: { name: "Toggle Group", id: "4374:2291" }
    set: { name: "ToggleGroup", id: "4378:2329" }   # 4 Member; jeder nestet 3 ECHTE Toggle-Instanzen (4375:2379-Member; B4/B9 — verifiziert 12 Instanzen, keine detached)
    members:
      "variant=default, spacing=gapped":    "(itemSpacing 2)"
      "variant=default, spacing=connected": "(itemSpacing 0)"
      "variant=outline, spacing=gapped":    "(itemSpacing 2)"
      "variant=outline, spacing=connected": "(itemSpacing 0)"
    axis: { variant: [default, outline], spacing: [gapped, connected] }   # spacing = Figma-only Proxy für das numerische Code-Prop `spacing` (0=connected, >0=gapped); NICHT als Enum-Prop zurücksyncen. orientation+size NICHT als Figma-Achse modelliert (Code trägt beide Props voll; orientation in der Vertical-Story, size via Toggle-Set-size-Achse).
    examples: { group: "Usage Examples 4379:2347 (Toolbar gapped / Segmented bar connected / Ghost group)" }
  skill: /shadcn-component-port (+ /figma-build-rules, 2026-06-22)
  notes: >
    Komposition Root + Item (Radix ToggleGroup, radix-ui Umbrella). HARTE Dep auf Toggle (Item importiert
    toggleVariants aus @/components/ui/toggle) → Toggle co-portiert (B10, wie Label↔Field). Context propagiert
    variant/size/spacing/orientation Root→Item (ToggleGroup-Idiom). type single|multiple = Radix-Root-Prop
    (single = radio-Semantik role=radio, multiple = Toggle-Buttons aria-pressed) — KEIN CVA. spacing>0 = getrennte
    Pills, spacing=0 = connected/segmented Bar (Item verliert Innenecken + Doppel-Border, nur Außenecken rund via
    first/last:corner-*-lg + corner-none). TYPECHECK-FALLE: Radix ToggleGroup.Root-Props sind eine DISKRIMINIERTE
    UNION auf `type` → `interface extends` über eine Union wirft TS2312; stattdessen `type … = ComponentProps<Root>
    & OwnProps`-Intersection (Item ist Objekt-Typ → interface extends ok). Figma: 4-Member-Set (variant×spacing),
    jeder nestet 3 echte Toggle-Instanzen (Item1 on) — B4/B9 reuse-not-rebuild, getNodeByIdAsync+createInstance.
    Connected-Innenecken in Figma = Annäherung (Items touchen mit eigenen runden Ecken; echtes Flatten der
