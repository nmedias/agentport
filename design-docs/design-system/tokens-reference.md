...
- **Semantics come from a source — never guessed.** `use` = the generic role of the token and its
  ...
  Where the source is silent: `tbd` or omit the field. A sentence leads with the abstract role and
  may then illustrate it with generic UI vocabulary ("row hover", "menus", "keycaps"); it never
  names a screen or a concrete instance — roles outlive both.
...
- **`use` is the canonical sentence** — the same text sits on the Figma variable / style description
  ...
  the `figma_suffix`. Every token that gets a foundations swatch shows its `use` — pages never omit
  it and never paraphrase it.
- **`scopes` is canonical here** — Figma vocabulary (`FRAME_FILL`, `SHAPE_FILL`, `TEXT_FILL`,
  `STROKE_COLOR`, `EFFECT_COLOR`, `CORNER_RADIUS`, `GAP`, `OPACITY`), verbatim, in this order. It says
  where a token may be applied and nothing else. Figma and the foundations pages follow this field —
  when they disagree, this file wins and the other two are corrected. Never restate a scope in prose.
...
```
token | css_var | primitive | value | utilities | scopes | use | avoid? | status? | note?
```
...
- { token: surface,     css_var: --ap-sys-surface,     primitive: base/white,  value: "#ffffff", utilities: [bg-surface],          scopes: [FRAME_FILL, SHAPE_FILL], use: "App base surface." }
...
- { token: card-fill,   css_var: --ap-sys-card-fill,   primitive: neutral/50,  value: "#f3f5fa", utilities: [bg-card-fill],        scopes: [FRAME_FILL, SHAPE_FILL], use: "Raised / secondary panel surface." }
- { token: card-ink,    css_var: --ap-sys-card-ink,    primitive: neutral/900, value: "#0d1016", utilities: [text-card-ink],       scopes: [SHAPE_FILL, TEXT_FILL], use: "Text on card-fill." }
- { token: muted-fill,  css_var: --ap-sys-muted-fill,  primitive: neutral/25,  value: "#f9fcfd", utilities: [bg-muted-fill],       scopes: [FRAME_FILL, SHAPE_FILL], use: "Low-emphasis surface that recedes behind content — quiet strips, row hover, quiet variants of a control. Content panels use card-fill instead." }
- { token: muted-ink,   css_var: --ap-sys-muted-ink,   primitive: neutral/500, value: "#656971", utilities: [text-muted-ink],      scopes: [SHAPE_FILL, TEXT_FILL], use: "Text / icon on muted-fill only. For de-emphasised text on any other surface use muted." }
- { token: muted,       css_var: --ap-sys-muted,       primitive: neutral/500, value: "#656971", utilities: [text-muted, bg-muted, fill-muted], scopes: [SHAPE_FILL, TEXT_FILL], use: "De-emphasised text, icon or marker on surfaces other than muted-fill — descriptions, hints, group headings, secondary glyphs. Shape fill only — no frame fill (surfaces use muted-fill).", note: "Scopes SHAPE_FILL + TEXT_FILL, no FRAME_FILL → bg-muted / fill-muted only for shape / marker fills." }

# Primary / secondary / accent (Figma: Primary/ · Secondary/ · Accent/)
- { token: primary,        css_var: --ap-sys-primary,        primitive: signal/600, value: "#0063bb", utilities: [text-primary, border-primary, ring-primary, bg-primary, fill-primary], scopes: [SHAPE_FILL, TEXT_FILL, STROKE_COLOR], use: "Emphasis colour for interactive text and glyphs on light surfaces (AA on white) — links, link-style actions, caret / marker shapes. Shape fill only — no frame fill (surfaces use primary-fill). On dark surfaces use brand-ink; for state tints use accent-fill.", note: "Scopes SHAPE_FILL + TEXT_FILL + STROKE_COLOR, no FRAME_FILL → bg-primary / fill-primary only for shape / marker fills (a rectangle / vector in Figma — e.g. the command caret), never as a container surface." }
- { token: primary-fill,   css_var: --ap-sys-primary-fill,   primitive: deep/900,   value: "#0d2531", utilities: [bg-primary-fill, border-primary-fill],  scopes: [FRAME_FILL, SHAPE_FILL, STROKE_COLOR], use: "Dark surface of the main action and of the checked / on state of a control (check box, radio dot, switch track, filled range). Also the edge of that same area, so fill and edge read as one form. Pairs with primary-ink." }
- { token: primary-ink,    css_var: --ap-sys-primary-ink,    primitive: signal/100, value: "#a4e5ff", utilities: [text-primary-ink, bg-primary-ink], scopes: [SHAPE_FILL, TEXT_FILL], use: "Text, icon or marker on primary-fill only (the marker inside a control, e.g. a radio dot)." }
- { token: secondary-fill, css_var: --ap-sys-secondary-fill, primitive: still/100,  value: "#bde4fd", utilities: [bg-secondary-fill], scopes: [FRAME_FILL, SHAPE_FILL], use: "Light surface of the secondary action — secondary buttons, badges. Lower weight than primary-fill; for quiet chrome use muted-fill. Pairs with secondary-ink." }
- { token: secondary-ink,  css_var: --ap-sys-secondary-ink,  primitive: deep/900,   value: "#0d2531", utilities: [text-secondary-ink], scopes: [SHAPE_FILL, TEXT_FILL], use: "Text / icon on secondary-fill only." }
- { token: accent-fill,    css_var: --ap-sys-accent-fill,    primitive: deep/50,    value: "#eaf8ff", utilities: [bg-accent-fill],   scopes: [FRAME_FILL, SHAPE_FILL], use: "Tint that marks state — selected rows, active items, hover on list entries. Not an action surface (that is secondary-fill / primary-fill). Pairs with accent-ink and accent-border." }
- { token: accent-ink,     css_var: --ap-sys-accent-ink,     primitive: signal/600, value: "#0063bb", utilities: [text-accent-ink],  scopes: [SHAPE_FILL, TEXT_FILL], use: "Text / icon on accent-fill only." }
- { token: accent-border,  css_var: --ap-sys-accent-border,  primitive: still/200,  value: "#9fcdeb", utilities: [border-accent-border], scopes: [STROKE_COLOR], use: "Edge of an accent-fill area — outlines the selected / active item. Not a focus ring (use ring)." }

# Brand (Figma: Brand/) — on-dark brand moment
- { token: brand-fill, css_var: --ap-sys-brand-fill, primitive: deep/900,   value: "#0d2531", utilities: [bg-brand-fill], scopes: [FRAME_FILL, SHAPE_FILL, EFFECT_COLOR], use: "Dark surface reserved for brand moments — hero, intro, wordmark panels. Not for functional dark chrome (use inverse-fill). Pairs with brand-ink." }
- { token: brand-ink,  css_var: --ap-sys-brand-ink,  primitive: signal/400, value: "#009fe3", utilities: [text-brand-ink, bg-brand-ink, fill-brand-ink], scopes: [SHAPE_FILL, TEXT_FILL, STROKE_COLOR, EFFECT_COLOR], use: "Signal-blue text, icon or marker on brand-fill only. On light surfaces use primary. Shape fill only — no frame fill.", note: "Scopes SHAPE_FILL + TEXT_FILL + STROKE_COLOR, no FRAME_FILL → bg-brand-ink / fill-brand-ink only for shape / marker fills." }

# Destructive (Figma: Destructive/)
- { token: destructive,     css_var: --ap-sys-destructive,     primitive: error/600, value: "#b01207", utilities: [bg-destructive, text-destructive, border-destructive, ring-destructive], scopes: [FRAME_FILL, SHAPE_FILL, TEXT_FILL, STROKE_COLOR], use: "Colour of irreversible actions and errors — delete buttons, invalid-field borders, error text, its focus ring. One token for fill, text and stroke. Not for warnings. Pairs with destructive-ink when used as a surface." }
- { token: destructive-ink, css_var: --ap-sys-destructive-ink, primitive: error/50,  value: "#ffe3d9", utilities: [text-destructive-ink, border-destructive-ink, bg-destructive-ink], scopes: [SHAPE_FILL, TEXT_FILL, STROKE_COLOR], use: "Text, icon, edge or marker on a destructive surface only (e.g. the dot of an invalid radio)." }

# Ring + borders (Figma: Focus/ · Border/) — line ladder ascending: border < border-emphasis < border-strong
- { token: ring,            css_var: --ap-sys-ring,            primitive: neutral/800, value: "#1e2229", utilities: [ring-ring, outline-ring, border-ring],   scopes: [STROKE_COLOR], use: "Keyboard-focus indicator on light surfaces — drawn as the focus border of the control plus its ring/50 outline, and it replaces the resting edge for as long as focus lasts. Only for focus: the resting edge stays border or input-border, the selected / active edge is accent-border." }
- { token: border,          css_var: --ap-sys-border,          primitive: neutral/75,  value: "#e4e6eb", utilities: [border-border, bg-border],   scopes: [SHAPE_FILL, STROKE_COLOR], use: "Default edge — dividers, card and field outlines on light surfaces. Start here; step up only when a line must read stronger." }
- { token: border-emphasis, css_var: --ap-sys-border-emphasis, primitive: neutral/200, value: "#b8bbc0", utilities: [border-border-emphasis],     scopes: [SHAPE_FILL, STROKE_COLOR], use: "Second step of the line ladder — table header rules, group separators that must stand out from border." }
- { token: border-strong,   css_var: --ap-sys-border-strong,   primitive: neutral/300, value: "#9b9fa5", utilities: [border-border-strong],       scopes: [SHAPE_FILL, STROKE_COLOR], use: "Top step of the line ladder — the one line that must dominate (axis, hard cut). Use sparingly." }

# Sidebar (Figma: Sidebar/) — no sidebar component exists yet; the family mirrors the main layer for a future one
- { token: sidebar-fill,         css_var: --ap-sys-sidebar-fill,         primitive: neutral/25,  value: "#f9fcfd", utilities: [bg-sidebar-fill],          scopes: [FRAME_FILL, SHAPE_FILL], use: "Surface of the navigation sidebar / rail. Only inside the sidebar; elsewhere use surface or muted-fill." }
- { token: sidebar-ink,          css_var: --ap-sys-sidebar-ink,          primitive: neutral/900, value: "#0d1016", utilities: [text-sidebar-ink],         scopes: [SHAPE_FILL, TEXT_FILL], use: "Default text / icon on sidebar-fill." }
- { token: sidebar-primary-fill, css_var: --ap-sys-sidebar-primary-fill, primitive: deep/900,    value: "#0d2531", utilities: [bg-sidebar-primary-fill, text-sidebar-primary-fill],  scopes: [FRAME_FILL, SHAPE_FILL, TEXT_FILL], use: "Surface of the main sidebar action (e.g. the workspace / brand button); also usable as its text / icon colour. Pairs with sidebar-primary-ink." }
- { token: sidebar-primary-ink,  css_var: --ap-sys-sidebar-primary-ink,  primitive: signal/200,  value: "#7cceff", utilities: [text-sidebar-primary-ink], scopes: [SHAPE_FILL, TEXT_FILL], use: "Text / icon on sidebar-primary-fill only." }
- { token: sidebar-accent-fill,  css_var: --ap-sys-sidebar-accent-fill,  primitive: deep/50,     value: "#eaf8ff", utilities: [bg-sidebar-accent-fill],   scopes: [FRAME_FILL, SHAPE_FILL], use: "Tint of the active / hovered navigation item. Pairs with sidebar-accent-ink." }
- { token: sidebar-accent-ink,   css_var: --ap-sys-sidebar-accent-ink,   primitive: signal/600,  value: "#0063bb", utilities: [text-sidebar-accent-ink],  scopes: [SHAPE_FILL, TEXT_FILL], use: "Text / icon on sidebar-accent-fill only." }
- { token: sidebar-border,       css_var: --ap-sys-sidebar-border,       primitive: neutral/50,  value: "#f3f5fa", utilities: [border-sidebar-border],    scopes: [STROKE_COLOR], use: "Dividers and the sidebar edge." }
- { token: sidebar-ring,         css_var: --ap-sys-sidebar-ring,         primitive: neutral/800, value: "#1e2229", utilities: [ring-sidebar-ring],        scopes: [STROKE_COLOR], use: "Keyboard-focus indicator inside the sidebar." }

# Charts — ramp-bound (Figma: Charts/)
- { token: chart-1, css_var: --ap-sys-chart-1, primitive: warning/700, value: "#753100", utilities: [bg-chart-1, border-chart-1], scopes: [FRAME_FILL, SHAPE_FILL, STROKE_COLOR], use: "Data-series colour 1 of 5 — assign in order (series 1 → chart-1). Carries no status meaning even where the hue matches a status ramp; for errors use destructive." }
- { token: chart-2, css_var: --ap-sys-chart-2, primitive: success/600, value: "#298058", utilities: [bg-chart-2, border-chart-2], scopes: [FRAME_FILL, SHAPE_FILL, STROKE_COLOR], use: "Data-series colour 2 of 5 — assign in order (series 1 → chart-1). Carries no status meaning even where the hue matches a status ramp; for errors use destructive." }
- { token: chart-3, css_var: --ap-sys-chart-3, primitive: deep/900, value: "#0d2531", utilities: [bg-chart-3, border-chart-3], scopes: [FRAME_FILL, SHAPE_FILL, STROKE_COLOR], use: "Data-series colour 3 of 5 — assign in order (series 1 → chart-1). Carries no status meaning even where the hue matches a status ramp; for errors use destructive." }
- { token: chart-4, css_var: --ap-sys-chart-4, primitive: warning/400, value: "#c8923f", utilities: [bg-chart-4, border-chart-4], scopes: [FRAME_FILL, SHAPE_FILL, STROKE_COLOR], use: "Data-series colour 4 of 5 — assign in order (series 1 → chart-1). Carries no status meaning even where the hue matches a status ramp; for errors use destructive." }
- { token: chart-5, css_var: --ap-sys-chart-5, primitive: error/500, value: "#c54235", utilities: [bg-chart-5, border-chart-5], scopes: [FRAME_FILL, SHAPE_FILL, STROKE_COLOR], use: "Data-series colour 5 of 5 — assign in order (series 1 → chart-1). Carries no status meaning even where the hue matches a status ramp; for errors use destructive." }

# Dialog + scrim (Figma: Dialog/ · Scrim/) — one raised-surface token for everything floating
- { token: dialog-fill,         css_var: --ap-sys-dialog-fill,         primitive: base/white,  value: "#ffffff", utilities: [bg-dialog-fill, fill-dialog-fill], scopes: [FRAME_FILL, SHAPE_FILL], use: "Surface of anything floating above the layout — dialogs, popovers, menus, command palette, tooltips. Pairs with dialog-ink and the Elevation effect. For in-flow panels use card-fill." }
- { token: dialog-ink,          css_var: --ap-sys-dialog-ink,          primitive: neutral/900, value: "#0d1016", utilities: [text-dialog-ink],        scopes: [SHAPE_FILL, TEXT_FILL], use: "Default text / icon on dialog-fill." }
- { token: scrim,               css_var: --ap-sys-scrim,               primitive: "neutral/900 × scrim-opacity", value: "color-mix(in srgb, #0d1016 10%, transparent)", utilities: [bg-scrim], scopes: [FRAME_FILL, SHAPE_FILL], use: "Colour of the backdrop that dims the page behind a modal dialog. The strength is already composed in — never add an opacity modifier on top of bg-scrim." }
- { token: scrim-opacity,       css_var: --ap-sys-scrim-opacity,       primitive: opacity/10,  value: "10%", utilities: [], scopes: [OPACITY], use: "Strength of the modal backdrop (10 %) — composed into scrim, it has no utility of its own.", note: "FLOAT, scope OPACITY, alias → opacity/10; bound to the opacity of the overlay layer in Figma. No foundations swatch (not a colour)." }

# Input (Figma: Input/)
- { token: input-ink-placeholder, css_var: --ap-sys-input-ink-placeholder, primitive: neutral/500, value: "#656971", utilities: [text-input-ink-placeholder], scopes: [TEXT_FILL], use: "Placeholder / hint text inside a field. The entered value uses ink; helper text outside the field uses muted." }
- { token: input-fill,            css_var: --ap-sys-input-fill,            primitive: neutral/25,  value: "#f9fcfd", utilities: [bg-input-fill],               scopes: [FRAME_FILL, SHAPE_FILL], use: "Resting surface of an editable field — text inputs, selects, check boxes before they are checked. Pairs with ink for the value and input-ink-placeholder for the hint." }
- { token: input-fill-high,       css_var: --ap-sys-input-fill-high,       primitive: neutral/400, value: "#7f848b", utilities: [bg-input-fill-high],          scopes: [FRAME_FILL, SHAPE_FILL], use: "Resting track of a range or toggle control (the unfilled part). The filled / on part is primary-fill. Same tone as input-border so control and edge read as one." }
- { token: input-border,          css_var: --ap-sys-input-border,          primitive: neutral/400, value: "#7f848b", utilities: [border-input-border],         scopes: [STROKE_COLOR], use: "Edge of fields and controls (AA against surface). Deliberately stronger than border; for the focused state add ring." }

# Inverse (Figma: Inverse/) — dark functional surfaces
- { token: inverse-fill,            css_var: --ap-sys-inverse-fill,            primitive: deep/950,        value: "#00121c",   utilities: [bg-inverse-fill],            scopes: [FRAME_FILL, SHAPE_FILL], use: "Dark functional surface — a navigation rail, keycaps, dark chips. Not for brand moments (use brand-fill). Pairs with inverse-ink." }
- { token: inverse-ink,             css_var: --ap-sys-inverse-ink,             primitive: neutral/75,      value: "#e4e6eb",   utilities: [text-inverse-ink],           scopes: [TEXT_FILL], use: "Default text / icon on inverse-fill." }
- { token: inverse-ink-muted,       css_var: --ap-sys-inverse-ink-muted,       primitive: neutral/400,     value: "#7f848b",   utilities: [text-inverse-ink-muted],     scopes: [SHAPE_FILL, TEXT_FILL, STROKE_COLOR], use: "De-emphasised text / icon on inverse-fill — the dark-surface counterpart of muted." }
- { token: inverse-border,          css_var: --ap-sys-inverse-border,          primitive: deep/900,        value: "#0d2531",   utilities: [border-inverse-border],      scopes: [SHAPE_FILL, STROKE_COLOR], use: "Dividers and edges on inverse-fill." }
- { token: inverse-container,       css_var: --ap-sys-inverse-container,       primitive: "deep/900 @30%", value: "#0d25314d", utilities: [bg-inverse-container],       scopes: [FRAME_FILL, SHAPE_FILL], use: "Resting inner panel on inverse-fill (card in the rail).", note: "Raw RGBA: Figma cannot alias an alpha; CSS composes it via color-mix." }
- { token: inverse-container-low,   css_var: --ap-sys-inverse-container-low,   primitive: "deep/900 @20%", value: "#0d253133", utilities: [bg-inverse-container-low],   scopes: [FRAME_FILL, SHAPE_FILL], use: "Idle / inactive inner panel on inverse-fill.", note: "Raw RGBA, see inverse-container." }
- { token: inverse-container-hover, css_var: --ap-sys-inverse-container-hover, primitive: "deep/900 @70%", value: "#0d2531b2", utilities: [bg-inverse-container-hover], scopes: [FRAME_FILL, SHAPE_FILL], use: "Hovered / active inner panel on inverse-fill.", note: "Raw RGBA, see inverse-container." }
```
...
```yaml
- { token: corner-none, css_var: "—",                  primitive: "— (static)", value: 0px,   utilities: [corner-none], scopes: [CORNER_RADIUS], use: "Square — no radius.", note: "Static utility, no variable in Figma and no CSS var — the absence of a radius." }
- { token: corner-sm,   css_var: --ap-sys-corner-sm,   primitive: radius/4,    value: 4px,    utilities: [corner-sm],   scopes: [CORNER_RADIUS], use: "Smallest radius — tick boxes, keycaps, markers and rows nested inside a panel." }
- { token: corner-md,   css_var: --ap-sys-corner-md,   primitive: radius/6,    value: 6px,    utilities: [corner-md],   scopes: [CORNER_RADIUS], use: "Compact size class of a control (small / icon-only sizes), tooltips, menu rows." }
- { token: corner-lg,   css_var: --ap-sys-corner-lg,   primitive: radius/8,    value: 8px,    utilities: [corner-lg],   scopes: [CORNER_RADIUS], use: "Regular size class of a control — fields, standard buttons, floating panels, in-flow items. Default; start here." }
- { token: corner-xl,   css_var: --ap-sys-corner-xl,   primitive: radius/16,   value: 16px,   utilities: [corner-xl],   scopes: [CORNER_RADIUS], use: "Windows — dialogs and other large floating surfaces." }
- { token: corner-full, css_var: --ap-sys-corner-full, primitive: radius/full, value: 9999px, utilities: [corner-full], scopes: [CORNER_RADIUS], use: "Pills and circles — toggles, radio dots, slider parts, badges. Only for shapes meant to read as round." }
```
...
```yaml
- { token: space-2xs, css_var: --ap-sys-space-2xs, primitive: "— (direct)", value: 2px,  utilities: [p-2xs, gap-2xs], scopes: [GAP], use: "Hairline — vertical padding of the smallest pill, gap between stacked micro-elements." }
- { token: space-xs,  css_var: --ap-sys-space-xs,  primitive: space/base,   value: 4px,  utilities: [p-xs,  gap-xs],  scopes: [GAP], use: "Gap inside a control (icon to label); tightest inner padding." }
- { token: space-sm,  css_var: --ap-sys-space-sm,  primitive: "— (direct)", value: 6px,  utilities: [p-sm,  gap-sm],  scopes: [GAP], use: "Inner padding of small controls; gap in inline text runs." }
- { token: space-md,  css_var: --ap-sys-space-md,  primitive: "— (direct)", value: 8px,  utilities: [p-md,  gap-md],  scopes: [GAP], use: "Default — gap between siblings, standard control padding. Start here." }
- { token: space-lg,  css_var: --ap-sys-space-lg,  primitive: "— (direct)", value: 12px, utilities: [p-lg,  gap-lg],  scopes: [GAP], use: "Padding of list rows and floating panels; gap between a control and its label block." }
- { token: space-xl,  css_var: --ap-sys-space-xl,  primitive: "— (direct)", value: 16px, utilities: [p-xl,  gap-xl],  scopes: [GAP], use: "Padding of windows and panels; gap between groups." }
- { token: space-2xl, css_var: --ap-sys-space-2xl, primitive: "— (direct)", value: 24px, utilities: [p-2xl, gap-2xl], scopes: [GAP], use: "Vertical breathing room of empty / placeholder states." }
- { token: space-3xl, css_var: --ap-sys-space-3xl, primitive: "— (direct)", value: 32px, utilities: [p-3xl, gap-3xl], scopes: [GAP], use: "Reserved space for a trailing indicator inside a row." }
- { token: space-4xl, css_var: --ap-sys-space-4xl, primitive: "— (direct)", value: 48px, utilities: [p-4xl, gap-4xl], scopes: [GAP], use: "Page and section margins of a layout." }
- { token: space-5xl, css_var: --ap-sys-space-5xl, primitive: "— (direct)", value: 80px, utilities: [p-5xl, gap-5xl], scopes: [GAP], use: "Editorial / hero spacing only." }
```
...
- token: data-sm
  ...
  use: "Micro mono value — meta, slugs, counters. Same parts as eyebrow, which is the uppercase counterpart."
