...
| Nachtrag 2026-06-11: CSS-Naming `--ap-sys-*` | Semantic-CSS-Vars = `--ap-sys-<leaf>` (`--ap-sys-space-md`, `--ap-sys-radius-lg`); CSS-Helfer `--space-base` entfällt (Steps rechnen direkt mit `--ap-dimension-space-base`) | CSS-only; Figma-Namen unverändert. Details: token-analysis-color Entscheidungs-Log |
