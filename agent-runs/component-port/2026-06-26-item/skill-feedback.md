...
| Field | Value |
|---|---|
| Why A | User-found defect: "some elements have no tokens, see title." The Item title's text colour and the default media icon's fill were set to a raw hex (`#0d1217`), not bound to a DS variable — while *adjacent* elements (description, chevron) were correctly bound to `muted-ink`. |
...
| Field | Value |
|---|---|
| Why A | Two user-found defects from one root: (#1) "why was item media not used as a component?" — the Item `media` slot's default was a raw icon frame, not an instance of the `.ItemMedia` component I had built. (#5) "in item media the icon/image cannot be swapped" — the `.ItemMedia` set has only a `variant` prop, no slot, so its glyph/image are baked and un-swappable. |
...
| Field | Value |
|---|---|
| Why B | User-found inconsistency (#4): "item group is not a component. why." I built `Item` + `ItemMedia` as components but used a plain frame for `ItemGroup` in the example. ItemGroup is a real named part (role=list, responsive gap) but is layout-only and its `has-data-[size]` responsive gap can't be expressed in Figma. |
