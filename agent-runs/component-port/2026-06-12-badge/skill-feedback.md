...
**Verified:** built 6 components across 2 `use_figma` calls without a page-set → all 6 landed on page `✦✦✦✦ Final ✦✦✦✦` (first page) while the target Section was on `Components`; combine threw the same-page error. Re-parenting the components to the target page fixed it.
