...
//  input border) · text-base/md:text-sm → text-format-label (Figma DS uses the Label
...
function Input({ className, type, ...props }: React.ComponentProps<'input'>) {
  return (
    <input
      ...
      data-slot="input"
      className={cn(
        'h-8 w-full min-w-0 corner-lg border border-input bg-input-background px-md py-xs text-format-label transition-[color,box-shadow] outline-none selection:bg-primary selection:text-primary-foreground file:inline-flex file:h-6 file:border-0 file:bg-transparent file:text-format-label file:text-foreground placeholder:text-input-placeholder disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50',
