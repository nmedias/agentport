...
// (h-5/min-w-5/px-1→px-xs/gap-1→gap-xs/rounded-sm/size-3), so geometry stays
...
// (Space/space-xs), rounded-sm (Radius/radius-sm), tooltip context via v4
...
const kbdVariants = cva(
  "pointer-events-none inline-flex h-5 w-fit min-w-5 items-center justify-center gap-xs rounded-sm px-xs text-kbd select-none in-data-[slot=tooltip-content]:bg-background/20 in-data-[slot=tooltip-content]:text-background [&_svg:not([class*='size-'])]:size-3",
  {
    variants: {
      emphasis: {
